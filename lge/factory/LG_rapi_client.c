...
void msm_get_MEID_type(char* sMeid)
{
	...
	if (rc < 0)
	{
		printk("%s, rapi reqeust failed\r\n", __func__);
		memset(sMeid,0,15);
	}
	else
	{
		rc = lg_rapi_check_validity_and_copy_result((void*)&ret, (char*)sMeid, 15); // returned MEID size is 15		
		if(rc == LG_RAPI_INVALID_RESPONSE)
			memset(sMeid,0,15);
		else
	...
	printk(KERN_ERR "%s, output length =%d\n",__func__,*ret.out_len);	
