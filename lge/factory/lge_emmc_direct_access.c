...
#if 0  //                                                    
/*                                                                         */
extern int lge_webdload_init(void);
static int download_write_block(const char *val, struct kernel_param *kp)
{
	if(val == NULL)
	{
		printk(KERN_ERR "%s, NULL buf\n", __func__);
	...
	printk(KERN_INFO "%s [SATRT] \n", __func__);
	lge_webdload_init();	
	printk(KERN_INFO "%s [END] \n", __func__);

	return 0;
}
module_param_call(download_write, download_write_block, NULL, NULL, S_IWUSR | S_IRUSR | S_IWGRP | S_IRGRP | S_IROTH);
/*                                                                         */
#endif
