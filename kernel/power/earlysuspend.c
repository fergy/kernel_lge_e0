...
#ifdef CONFIG_MACH_MSM7X25A_V1	
	if(new_state == 3 && requested_suspend_state == 0 && stepSuspend == 0) 
	{
		stepSuspend = 1;
	}
	else if(new_state == 0 && stepSuspend == 2){
		 stepSuspend = 0;
	}

		pr_info("requested_suspend_state : %d,new_state : %d stepSuspend : %d\n", requested_suspend_state, new_state, stepSuspend);
#endif
