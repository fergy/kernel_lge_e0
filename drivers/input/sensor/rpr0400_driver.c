...
#define PS_ALS_SET_ALSPS_CONTROL  (/*LEDCURRENT_200MA*/LEDCURRENT_100MA | ALSGAIN_X128X128)	//Set high gain value to acquire high accuracy
...
/* effect calibrate */
#define HIGH_THRESHOLD_CAL 7//10//7//18//17//37	//(17)
#define LOW_THRESHOLD_CAL  5 //4//6//4//13//10//30	//(10)
/* prohibit calibrate */
#define HIGH_THRESHOLD_NON_CAL 17//(40)
#define LOW_THRESHOLD_NON_CAL  14//(35)
...
static int rpr400_read_crosstalk_data_fs(void)
{
	...
	if (fd >= 0) {
		size = sys_read(fd, read_buf, sizeof(read_buf));
		if(size != sizeof(read_buf))
		{
			printk(KERN_INFO"[%s] read file size is unmatched", __func__);
			data = -1;
		}
		else
		{
            printk(KERN_INFO"[%s] calibration file read success",__func__);
...
static int rpr400_backup_crosstalk_data_fs(unsigned int val)
{
	...
	snprintf(buf, CALB_CHAR_LEN, "%d", val);	//WBT #472480 Buffer Overflow, sprintf -> snprintf

	set_fs(KERNEL_DS);
	fd = sys_open(CALIB_FILE,O_WRONLY|O_CREAT, 0664);

	if(fd >= 0) {
		size = sys_write(fd, buf, sizeof(buf));
		sys_fsync(fd);
		if(size <= 0)
		{ 
            printk(KERN_INFO"[%s] sys_write FAIL file size = %d ",__func__,size);
			result = -1;
		}
		else
		{
            printk(KERN_INFO"[%s] sys_write success file size = %d ",__func__,size);
			result = 0;
		}
	} else {
		printk(KERN_ERR "file open error!!");
...
static void rpr400_make_threshold_value(struct i2c_client *client, unsigned int *high, unsigned int *low)
{
	...
	if(data < 0) {
		...
		*low  = LOW_THRESHOLD_NON_CAL;
		...
		if(data >= 300/*HIGH_THRESHOLD_NON_CAL*/)
		{
		...
		else
		{
			...
			*low  = data + LOW_THRESHOLD_CAL;
            printk(KERN_INFO"[%s] HIGH Threshold = %d ",__func__, *high);
            printk(KERN_INFO"[%s] LOW Threshold = %d ",__func__, *low);
		}
	}
					...
					struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for(i = 0; i < CALB_TIMES; i ++) {
		cross_talk = i2c_smbus_read_word_data(client, REG_PSDATA);
		if(cross_talk < 0) {
			printk(KERN_INFO"[%s] Calibration Error pdata value = %d ",__func__, tmp[i]);
			goto err_exit;
		}
		tmp[i] = cross_talk;
	...
	for(i=0; i<CALB_TIMES-1; i++)
		for(j=i+1; j<CALB_TIMES; j++)
			if(tmp[i] > tmp[j])
				rpr0400_swap(tmp+i, tmp+j);
	...
	if(cross_talk >= 300)
	{
		printk(KERN_INFO"[%s] Calibration Fail. Crosstalk Value : %d ",__func__, cross_talk);
		goto err_exit;
	}
	else
	{
		ret = rpr400_backup_crosstalk_data_fs(cross_talk);
		if(ret < 0)
		{
            printk(KERN_INFO"[%s] Fail backup data to FS ret= %d", __func__, ret);
			goto err_exit;
		}
		als_ps->calib_result = 1;
	}
	
    high = cross_talk + HIGH_THRESHOLD_CAL;
    low  = cross_talk + LOW_THRESHOLD_CAL;
    switch(val){
...
err_exit:
	als_ps->calib_result = 0;
	#if 0 /*I don't know why this code is needed.*/
    rpr400_set_enable(client, 0);
    if(als_ps->enable_ps_sensor == 1) {
        als_ps->enable_ps_sensor = 0;
...
static int rpr400_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	disable_irq(client->irq);
    cancel_delayed_work_sync(&als_ps->dwork);  //add for bug fix.
