...
#if 0
// daehyun.gil , 2012-11-19
			if ( (ts->pdata->role->key_type == TOUCH_HARD_KEY) && (ts->ts_data.curr_data[id].y_position >= 1870) )
				continue;
// daehyun.gil , 2012-11-19
#endif
...
static int touch_work_pre_proc(struct lge_touch_data *ts)
{
	...
	if(likely(ts->pdata->role->operation_mode == 1))
		ts->int_pin_state = gpio_get_value(ts->pdata->int_pin);
	...
	if (ts->ts_data.state != TOUCH_ABS_LOCK) {
		if (!ts->ts_data.total_num) {
			...
			if(ts->ts_data.prev_button.state == BUTTON_PRESSED && ts->ts_data.state == TOUCH_BUTTON_LOCK) {
				...
				if (likely(touch_debug_mask & (DEBUG_BUTTON | DEBUG_BASE_INFO)))
					printk("Touch KEY[%d] is canceled\n",
							ts->ts_data.prev_button.key_code);
 ...
 * 6. report period
 */
static int check_platform_data(struct syna_touch_platform_data *pdata)
{
	if (unlikely(touch_debug_mask & DEBUG_TRACE))
		TOUCH_DEBUG_MSG("\n");
 ...
 */
void get_section(struct section_info* sc, struct syna_touch_platform_data *pdata)
{
	int i;
...
{
	struct syna_touch_platform_data *pdata = ts->pdata;
	int ret = 0;
