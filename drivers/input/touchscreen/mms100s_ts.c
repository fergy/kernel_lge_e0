...
static void mms_ts_disable(struct mms_ts_info *info)
{
	...
	*/
	i2c_smbus_write_byte_data(info->client, MMS_OSC_OFF, 0x1);  // OSC off command
	msleep(10);
...
static void mms_report_input_data(struct mms_ts_info *info, u8 sz, u8 *buf)
{
	...
	for (i = 0; i < sz; i += FINGER_EVENT_SZ) {
		...
		if (tmp[0] & MMS_TOUCH_KEY_EVENT) {
			...
			dev_info(&client->dev, "key event - key_code : %d, key_state : %d\n", key_code, key_state);
			...
			pressure = tmp[5];
			if (pressure == 0) {
				dev_dbg(&client->dev, "id:%d, x:%d, y:%d, tourch_major:%d, pressure:%d\n", id, x,y,touch_major,pressure);
...
static int mms_flash_section(struct mms_ts_info *info, struct mms_fw_img *img, const u8 *data)
{
	...
	if(!isc_packet) 	//WBT #472479 Null Pointer Dereference 'isc_packet'
		return -ENOMEM;	
...
static int mms_flash_fw(const struct firmware *fw, struct mms_ts_info *info)
{
	...
	if (update_flag) {
		msleep(5);
		mms_reboot(info);	//2013-04-12 reboot is required only when firmware update done
			if (get_fw_version(client, ver)) {
...
static void mms_flash_fw_controller(const struct firmware *fw, void *context)
{
	int retires = 3;
	int ret;
	u8 ver[MAX_SECTION_NUM] = {0,}; //WBT #472476 uninitialized local variable
	...
	dev_dbg(&info->client->dev, "touch firmware checking.\n");
	...
	dev_dbg(&info->client->dev, "touch firmware is updated.\n");	
}
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	//Exception handling when I2C Fail Case, retry 10 times
