			    ...
			    const char *buf, size_t size)
{
	...
	printk("[ TSP ] Calibration wait time %dsec\n", calib_ms_delay / 10);
			    ...
			    const char *buf, size_t size)
{
	...
	printk("[ TSP ] Use idle: %d, Use debug: %d\n", idle_en, debug_en);
			     ...
			     const char *buf, size_t size)
{
	...
	printk("[ TSP ] Request reset error count: %d\n", err_cnt);
...
# define MASK_SINGLE_TOUCH       (0x01)
# define MASK_MULTI_TOUCH        (0x02)

# define MAX_TRACKING_COUNT      (50)
struct timespec t_curr;      // ns

/* sysfs: /sys/class/touch/tunes/track */
ssize_t ist30xx_track_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	const int msg_len = 64;
	char msg[msg_len];
	int i, j, buf_cnt = 0;
	int finger_cnt, key_cnt;
	int track_cnt = MAX_TRACKING_COUNT;
	u32 ms = 0, status = 0;
	finger_info *finger = (finger_info *)&status;

	ktime_get_ts(&t_curr);
	ms = t_curr.tv_sec * 1000 + t_curr.tv_nsec / 1000000;
	...
	buf_cnt += snprintf(msg, msg_len, "ms(%d), num: %d of %d\n",
			    ms, track_cnt, ist30xx_get_track_cnt());
	strncat(buf, msg, msg_len);
	...
	for (i = 0; i < track_cnt; i++) {
		...
		if ((status & CALIB_MSG_MASK) == CALIB_MSG_VALID) {
			buf_cnt += snprintf(msg, msg_len, "%d: 0x%08x (%d, %d)\n",
					    ms, status, CALIB_TO_STATUS(status), CALIB_TO_GAP(status));
			strncat(buf, msg, msg_len);
		} else if ((finger->bit_field.udmg & MASK_MULTI_TOUCH) == MASK_MULTI_TOUCH) {
			buf_cnt += snprintf(msg, msg_len, "%d: 0x%08x (k%d, f%d)\n",
					    ms, status, finger->bit_field.x, finger->bit_field.y);
			strncat(buf, msg, msg_len);
			key_cnt = finger->bit_field.x;
			finger_cnt = finger->bit_field.y;
			for (j = 0; j < finger_cnt; j++, i++) {
				ist30xx_get_track(&ms, &status);
				finger = (finger_info *)&status;
				buf_cnt += snprintf(msg, msg_len, "%d- 0x%08x %d(%d, %d)\n",
						    ms, status, finger->bit_field.id,
						    finger->bit_field.x, finger->bit_field.y);
				strncat(buf, msg, msg_len);
			}
			for (j = 0; j < key_cnt; j++, i++) {
				ist30xx_get_track(&ms, &status);
				finger = (finger_info *)&status;
				buf_cnt += snprintf(msg, msg_len, "%d- 0x%08x %d(%d, %d)\n",
						    ms, status, finger->bit_field.id,
		...
		} else if ((finger->bit_field.udmg & MASK_SINGLE_TOUCH) == MASK_SINGLE_TOUCH) {
			buf_cnt += snprintf(msg, msg_len, "%d: 0x%08x %d(%d, %d)\n",
					    ms, status, finger->bit_field.id,
					    finger->bit_field.x, finger->bit_field.y);
			strncat(buf, msg, msg_len);
		} else {
			buf_cnt += snprintf(msg, msg_len, "%d: 0x%08x\n", ms, status);
			strncat(buf, msg, msg_len);
		}
...
/* sysfs: /sys/class/touch/tunes/track_frame */
...
/* sysfs: /sys/class/touch/tunes/track_cnt */
ssize_t ist30xx_track_cnt_show(struct device *dev, struct device_attribute *attr,
			       char *buf)
{
	u32 track_cnt = ist30xx_get_track_cnt();

	buf = (char *)&track_cnt;

	printk("[ TSP ] cnt: %d, buf[]: %02d%02d%02d%02d\n",
	       track_cnt, buf[0], buf[1], buf[2], buf[3]);

	return sizeof(track_cnt);
}
...
#if IST30XX_TRACKING_MODE
static DEVICE_ATTR(track, S_IRWXUGO, ist30xx_track_show, NULL);
...
#ifdef IST30XX_NODE_TEST
	&dev_attr_node_raw.attr,
	&dev_attr_node_base.attr,
#endif
	NULL,
};
...
static struct attribute_group internal_attr_group = {
...
int ist30xx_init_misc_sysfs(void)
{
	...
	/* /sys/class/touch/internal/... */
	if (sysfs_create_group(&ist30xx_internal_dev->kobj, &internal_attr_group))
