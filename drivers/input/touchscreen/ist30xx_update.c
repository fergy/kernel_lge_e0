...
int ist30xx_calib_wait(void)
{
	...
	while (cnt-- > 0) {
		...
		if (ts_data->status.calib_msg) {
			printk("[ TSP ] Calibration Status : %d, Max raw gap : %d - (raw: %08x)\n",
...
int ist30xx_calibrate(int wait_cnt)
{
	...
	printk("[ TSP ] *** Calibrate time %dsec ***\n", calib_ms_delay / 10);
...
int ist30xx_get_tkey_info(void)
{
	...
	ist30xx_get_update_info(ts_data, ist30xx_fw, sizeof(ist30xx_fw));
	cfg_buf = (u8 *)&ist30xx_fw[ts_data->tags.cfg_addr];
...
int ist30xx_get_tsp_info(void)
{
	...
	ist30xx_get_update_info(ts_data, ist30xx_fw, sizeof(ist30xx_fw));
	cfg_buf = (u8 *)&ist30xx_fw[ts_data->tags.cfg_addr];
	sensor_buf = (u8 *)&ist30xx_fw[ts_data->tags.sensor1_addr];
	...
	tsp->dir.txch_y = (bool)(sensor_buf[0x78] & 1 ?  true : false);
...
//	ist30xx_reset();
	...
	printk("[ TSP ] F/W version core: %08x, param: %08x\n",
...
#define calib_update_next_step(ret)   { if (ret) { step = 3; goto end; } }
...
//	ist30xx_reset();
	...
	update_next_step(ret);
	ts_data->fw.ver = ver;
	printk("[ TSP ] F/W core version : %08x\n", ts_data->fw.ver);
	...
	update_next_step(ret);
	ts_data->param_ver = ver;
	printk("[ TSP ] Param version : %08x\n", ts_data->param_ver);

end:
	if (ret) {
		if (step == 1)
			pr_err("[ TSP ] Firmware update Fail!, ret=%d", ret);
		else if (step == 2)
			pr_err("[ TSP ] Parameters update Fail!, ret=%d", ret);
		else if (step == 3)
			pr_err("[ TSP ] Calibration Fail!, ret=%d", ret);
	} else if (chksum != ts_data->fw.chksum) {
		pr_err("[ TSP ] Error CheckSum: 0x%08x, 0x%08x\n",
		       ts_data->fw.chksum, chksum);
		ret = -ENOEXEC;
	}
...
int ist30xx_auto_fw_update(struct ist30xx_data *data)
{
	...
	printk("[ TSP ] F/W version is new. 0x%08x -> 0x%08x\n",
...
int ist30xx_auto_param_update(struct ist30xx_data *data)
{
	...
	printk("[ TSP ] Param version is new. 0x%04x -> 0x%04x\n",
	     data->param_ver, val);
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	if ((data->param_ver > 0) && (data->param_ver < 0xFFFFFFFF) &&
		...
		ist30xx_fw_ver = ist30xx_parse_fw_ver(PARSE_FLAG_FW, ist30xx_fw);
		ist30xx_param_ver = ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xx_fw);

		printk("[ TSP ] F/W: %08x(%08x), Param: %04x(%04x)\n",
	...
	printk("[ TSP ] Update version F/W: %08x -> %08x, Param: %04x -> %04x\n",
	     data->fw.ver, ist30xx_fw_ver, data->param_ver, ist30xx_param_ver);
	...
	while (retry--) {
		if (isp_mode)
			ret = ist30xx_fw_update(data->client,
						ist30xx_fw, sizeof(ist30xx_fw), true);
		else
			ret = ist30xx_fw_param_update(data->client, ist30xx_fw);
		if (!ret)
			break;
	}
	mutex_unlock(&ist30xx_mutex);
			   ...
			   char *buf)
{
...
#if IST30XX_INTERNAL_BIN
	{
		...
		count += snprintf(msg, sizeof(msg),
				  ...
				  ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xx_fw));
