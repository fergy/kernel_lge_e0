...
/*                                  */
static bool v4l2_tavarua_state = false;
static int tavarua_vidioc_test_close(struct file *file);
/*                                */
...
/*                                  */
    printk("##>>> [jw.park] %s : v4l2_fm_disalbe:%d\n", __func__, v4l2_tavarua_state);
    if (v4l2_tavarua_state) {
        tavarua_vidioc_test_close(file);
    }
/*                                */
...
/*                                  */
                printk("###>>> [jw.park] V4L2_CID_PRIVATE_TAVARUA_STATE ctrl->value:%d\n", ctrl->value);
                if (ctrl->value==FM_RECV && retval>=0 ) {
                    printk("###>>> [jw.park] V4L2_CID_PRIVATE_TAVARUA_STATE v4l2_tavarua_state false set!!\n");
                    v4l2_tavarua_state = true;
                }
/*                                */
			}
		}
...
{
	int retval;
	unsigned char int_ctrl[XFR_REG_NUM] ={0,} ; 

	if (!radio->lp_mode) {
		FMDERR("%s: All interrupts are enabled already\n", __func__);
		return 0;
	}
    ...
    if (radio->chipID == MARIMBA_2_1 || radio->chipID == BAHAMA_1_0
		...
		FMDERR("%s: Setting interrupts for Chip : %x\n", __func__,
			radio->chipID);
...
static int tavarua_disable_interrupts(struct tavarua_device *radio)
{
	unsigned char lpm_buf[XFR_REG_NUM] = {0,};
	int retval;
	if (radio->lp_mode)
		return 0;
	FMDBG("%s\n", __func__);
...
{
	struct tavarua_device *radio = video_get_drvdata(video_devdata(file));
	struct  v4l2_control tmpctrl;
	tmpctrl.id = V4L2_CID_PRIVATE_TAVARUA_STATE;
	tmpctrl.value = FM_OFF;
	
	printk("##>>> [jw.park] %s : radio->registers[RDCTRL] : 0x%x\n", __func__, radio->registers[RDCTRL]);
	printk("##>>> [jw.park] %s : start tavarua_vidioc_s_ctrl , v4l2_tavarua_state :%d  !!!###\n", __func__, v4l2_tavarua_state);
	tavarua_vidioc_s_ctrl(file, NULL, &tmpctrl);
	printk("##>>> [jw.park] %s : end tavarua_vidioc_s_ctrl , v4l2_tavarua_state:%d !!!###\n", __func__, v4l2_tavarua_state);
	return 0;
}
/*                                */
