		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	int rc = 0;
	int retry = 0;
//	printk("[aidan_Camera]%s is called enum num: %d , value = %d\n", __func__, ctrl_info->ctrl_id, value);
...
int8_t t4k28_get_snapshot_data(struct msm_sensor_ctrl_t *s_ctrl, struct snapshot_soc_data_cfg *snapshot_data) {
	...
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3561,&analogGain1, MSM_CAMERA_I2C_BYTE_DATA);//                                                                       
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3562,&analogGain2, MSM_CAMERA_I2C_BYTE_DATA);//                                                                       
	...
	//Exposure Time 
	/*                                                               */
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x355f, &Exposure1, MSM_CAMERA_I2C_BYTE_DATA);
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3560, &Exposure2, MSM_CAMERA_I2C_BYTE_DATA);
	/*                                                              */
	//pr_err("[aidan]%s : Exposure time -  Exposure1 = 0x%x ",  __func__,  Exposure1);
	//pr_err("[aidan]%s : Exposure time -  Exposure2 = 0x%x ",  __func__,  Exposure2);
	if (rc < 0) {
	...
	//pr_err("[aidan]%s : iso speed - analogGain1 = 0x%x ",  __func__, analogGain1);
	//pr_err("[aidan]%s : iso speed - analogGain2 = 0x%x ",  __func__, analogGain2);
	//pr_err("[aidan]%s : iso speed - analogGain = 0x%x ",  __func__, analogGain);
	isoSpeed = ((analogGain / 32) * 100);
	//Exposure Time
	ExposureTotal = ((Exposure1<<8)|Exposure2);//                                                             
	//pr_err("[aidan]%s : Exposure time -  ExposureTotal = %d ",  __func__,  ExposureTotal);
	//pr_err("[aidan]%s : Exposure time -  ExposureTotal = %d ",  __func__,  ExposureTotal);
	if (ExposureTotal <= 0) {
		exposureTime = 600000;
	}else {
	...
	//pr_err("[aidan]%s : Exposure time -  ExposureTotal = %d ",  __func__,  exposureTime);
			...
			int update_type, int res)
{
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		if (res ==0)
		printk("[aidan]msleep time start.\n");	
			msleep(120);
		printk("[aidan]msleep time End.\n");	
