...
/*                                              
                   ...
                   */
#ifdef CONFIG_ANDROID_LOGGER_DUMPER
static LIST_HEAD(aux_logger_devices);
static DEFINE_MUTEX(devicelist_mtx);
static int uart_enabled = -1;
static int is_logger_mask_ok = -1;
extern int android_log_dumper_enable(void);
extern int android_logger_mask(void);
#endif
/*                                            */
...
#ifdef CONFIG_ANDROID_LOGGER_DUMPER
	if( -1 == uart_enabled )
		uart_enabled = android_log_dumper_enable();
	if( -1 == is_logger_mask_ok)
		is_logger_mask_ok = android_logger_mask();
	if(uart_enabled && is_logger_mask_ok)
