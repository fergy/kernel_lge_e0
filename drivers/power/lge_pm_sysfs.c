...
void check_miniabb_vendor(void)
{
	int vendor=-1;
	
	vendor = lge_get_miniabb_vendor();

	if(MINIABB_RT8965 == vendor)
		printk(KERN_INFO "M4=> Using Richitek MiniAbb");
	else if(MINIABB_LP8728 == vendor)
		printk(KERN_INFO "M4=> Using TI MiniAbb");
	else
		printk(KERN_ERR "M4=>Didnt find MiniAbb!");
}
#endif
