...
#define GPIO_CAM_AVDD_EN    73
#define GPIO_CAM_IO_EN       74
...
//	GPIO_CFG(GPIO_CAM_IO_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
//	GPIO_CFG(GPIO_CAM_AVDD_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
if (!gpio_initialzed) {
	/*	rc = gpio_request(GPIO_CAM_PWDN, "hi351_pwdn");
		...
		if (rc < 0) {
			...
			rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAM_AVDD_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		if (rc < 0) {
			pr_err("%s:unable to enable GPIO_CAM_AVDD_EN for main camera!\n",
				 __func__);
		...
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_CAM_IO_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		if (rc < 0) {
			pr_err("%s:unable to enable GPIO_CAM_IO_EN for main camera!\n",
				 __func__);
			gpio_free(GPIO_CAM_IO_EN);
		}

		rc = gpio_request(GPIO_CAM_AVDD_EN, "hi351_avdd_en");
		if (rc < 0) {
			pr_err("%s: gpio_request(GPIO_CAM_AVDD_EN) failed\n", __func__);
		}
		
		rc = gpio_request(GPIO_CAM_IO_EN, "hi351_io_en");
		if (rc < 0) {
			pr_err("%s: gpio_request(GPIO_CAM_IO_EN) failed\n", __func__);
		}
...
if (vreg_en) {
		...
		rc = gpio_direction_output(GPIO_CAM_AVDD_EN, 1);
		if (rc < 0) {
			pr_err("%s: gpio_direction_output(GPIO_CAM_AVDD_EN, 1) failed(1)\n", __func__);
		}

		rc = gpio_direction_output(GPIO_CAM_IO_EN, 1);
		if (rc < 0) {
			pr_err("%s: gpio_direction_output(GPIO_CAM_IO_EN, 1) failed(2)\n", __func__);
		}
			
} 
else {

		rc = gpio_direction_output(GPIO_CAM_AVDD_EN, 0);
		if (rc < 0) {
			pr_err("%s: gpio_direction_output(GPIO_CAM_AVDD_EN, 0) failed(1)\n", __func__);
		}

		rc = gpio_direction_output(GPIO_CAM_IO_EN, 0);
		if (rc < 0) {
			pr_err("%s: gpio_direction_output(GPIO_CAM_IO_EN, 0) failed(2)\n", __func__);
		}
...
static struct msm_camera_sensor_info msm_camera_sensor_hi351_data = {
	.sensor_name    = "hi351",
	.sensor_reset_enable = 1,
	.sensor_reset           = GPIO_CAM_RESET, 		// GPIO_34
	.sensor_pwd 			= GPIO_CAM_PWDN, 		// GPIO_42	
	.pdata                  = &msm_camera_device_data_csi1[0],
