...
#ifdef CONFIG_LGE_SWITCH_GPIO
#include <sound/fsa8008.h>
#endif
#ifndef CONFIG_LGE_SWITCH_GPIO
#define CONFIG_LGE_SWITCH_GPIO_HW
#endif

#include <mach/pmic.h>

#if defined (LGE_MACH_MSM7X27A_U0)
...
static struct cad_endpoint cad_endpoints_list[] = {
	...
	CAD(FM_DIGITAL_BT_A2DP_SPKR, 69, \
			(SNDDEV_CAP_FM | SNDDEV_CAP_RX)),
	CAD(HANDSET_TCOIL_SPKR, 70, (SNDDEV_CAP_RX | SNDDEV_CAP_VOICE)),
	CAD(HANDSET_TCOIL_MIC, 71, (SNDDEV_CAP_TX | SNDDEV_CAP_VOICE)),
...
/* add orignal code */
/* ear sense driver */
#if defined(CONFIG_LGE_SWITCH_GPIO_HW)
static struct gpio_h2w_platform_data l4ii_cdma_h2w_data = {
	.gpio_detect = GPIO_EAR_SENSE,
	.gpio_button_detect = GPIO_BUTTON_DETECT,
	.gpio_mic_mode = GPIO_MIC_MODE,
};

static struct platform_device l4ii_cdma_h2w_device = {
	.name = "lge-switch-gpio",
	.id = -1,
	.dev = {
		.platform_data = &l4ii_cdma_h2w_data,
	},
};


#elif defined(CONFIG_LGE_SWITCH_GPIO)
...
static int l4ii_cdma_gpio_earsense_work_func(int *value)
{
	int state;
	int gpio_value;
...
static char *l4ii_cdma_gpio_earsense_print_name(int state)
{
	if (state)
		return "Headset";
	else
...
static char *l4ii_cdma_gpio_earsense_print_state(int state)
{
...
static int l4ii_cdma_gpio_earsense_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned l4ii_cdma_earsense_gpios[] = {
	GPIO_EAR_SENSE,
};

/* especially to address gpio key */
static unsigned l4ii_cdma_hook_key_gpios[] = {
	GPIO_BUTTON_DETECT,
};

static int l4ii_cdma_gpio_hook_key_work_func(int *value)
{
	int gpio_value;
...
static struct lge_gpio_switch_platform_data l4ii_cdma_earsense_data = {
	/*                                         */
	...
	.name = "h2w",
	/*                    */
	.gpios = l4ii_cdma_earsense_gpios,
	.num_gpios = ARRAY_SIZE(l4ii_cdma_earsense_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = l4ii_cdma_gpio_earsense_work_func,
	.print_name = l4ii_cdma_gpio_earsense_print_name,
	.print_state = l4ii_cdma_gpio_earsense_print_state,
	.sysfs_store = l4ii_cdma_gpio_earsense_sysfs_store,

	/* especially to address gpio key */
	.key_gpios = l4ii_cdma_hook_key_gpios,
	.num_key_gpios = ARRAY_SIZE(l4ii_cdma_hook_key_gpios),
	.key_work_func = l4ii_cdma_gpio_hook_key_work_func,
};

static struct platform_device l4ii_cdma_earsense_device = {
	.name	= "lge-switch-gpio",
	.id		= -1,
	.dev	= {
		.platform_data = &l4ii_cdma_earsense_data,
	},
};
#else
/*Not Support*/
...
static struct platform_device *l4ii_cdma_sound_devices[] __initdata = {
	&msm_device_snd,
	&msm_device_cad,
	&msm_device_adspdec,
#if defined(CONFIG_LGE_SWITCH_GPIO)
	&l4ii_cdma_earsense_device,
#elif defined(CONFIG_LGE_SWITCH_GPIO_HW)
	&l4ii_cdma_h2w_device,
#endif
	&asoc_msm_pcm,
	&asoc_msm_dai0,
	&asoc_msm_dai1,
...
#endif
	platform_add_devices(l4ii_cdma_sound_devices, ARRAY_SIZE(l4ii_cdma_sound_devices));
}
